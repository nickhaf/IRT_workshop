...
viewof gamma1 = Inputs.range(
  [0, 1], 
  {value: 0, step: .1, label: tex`\gamma_1`}
)
...
viewof gamma2 = Inputs.range(
  [0, 1], 
  {value: 0, step: .1, label: tex`\gamma_2`}
)
...
data = d3.range(-3, 3, 0.1).map((theta) => ({
  ...
  y2: line_function(alpha2, beta2, gamma2, theta)
}));
...
Plot.plot(
  {
    ...
    marks: [
      ...
      Plot.line(data, {x: "x", y: "y2", stroke: "#F4BA02"}),
    ]
  }
)
```
