...
:::{.callout-note}
## Übung
Bitte bearbeite die [Übung zum Einlesen von Daten](https://nickhaf.github.io/IRT_workshop/exercises/intro_r/introR.html).
:::

#  {background-image="images/butterfly.jpg" background-size="1225px"}

::: {.absolute left="5%" bottom="5%" style="color:#01364C;"}
...
:::{.callout-important}
## `NA`
Viele der Zeilen enthalten nur `NA` Werte. Das schauen wir uns gleich im Kapitel @sec-missings genauer an.
:::
...
- Alle Zeilen entfernen, die in der Spalte `Weight` ein `NA` haben.  
```{r, output.lines=3}
athletes_na <- athletes[!is.na(athletes$Weight), ]
head(athletes_na)
```

- ... und mit [tidyverse]{https://www.tidyverse.org/}:
```{r, output.lines=3}
library(tidyverse)
...
::: {.absolute right="5%" top="5%" style="color:#01364C;"}
Reshaping
:::
