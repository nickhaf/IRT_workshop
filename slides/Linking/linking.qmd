...
library(tidyverse)
qa <- readRDS(here::here("raw_data", "q_a.rds"))
...
qa_answers <- qa %>%
  ...
  select(T1, gender, answer, question_code) %>%
  pivot_wider(id_cols = c(T1, gender), names_from = question_code, values_from = answer) %>%
  select(-T1)

qa_answers_na <- qa_answers %>%
  drop_na(gender)

library(eRm)

m1 <- RM(select(qa_answers, -gender))
m1_na <- RM(select(qa_answers_na, -gender))
...
gender_diffs <- Waldtest(m1, splitcr = qa_answers$gender)
...
plot(gender_1_diffs, gender_2_diffs, xlim = c(-2, 2), ylim = c(-2, 2),
...
## Wir könnten jetzt das stärkste DIF Item entfernen und die Analyse nochmal durchführen. Lassen wir aber, let's take a quick look at rasch trees!
```
...
```{r}
# https://rdrr.io/cran/equateIRT/man/dif.test.html
## DAs wäre ganz cool, weil es klar macht, dass equated wird, und weil das paper auch recht fresh ist dazu. 
```
