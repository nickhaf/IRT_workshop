...
Ankeritems sind [gemeinsame Items]{.highlight}, die in beiden Testformen vorhanden sind. Hauptproblem bei der Auswahl: Sie sollten in beiden Gruppen nicht unterschiedlich funktionieren, es sollte also kein Differential Item Functioning (DIF) geben, siehe @sec-dif.
...
[DIF-Untersuchung ist damit auch eine Untersuchung der Testvalidität!]{.highlight}
...
## Warum funktioniert das nicht?
Wir haben am Anfang dieser Präsi gesehen, dass man die Werte aus versch. Kalibrierungen nich ohne weiteres vergleichen kann, da die Skalen arbiträr festgelegt werden. [Wir müssen also vorher linken, siehe @sec-linking !]{.highlight}
