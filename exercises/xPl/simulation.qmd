...
## Ein zufälliges Item simulieren
Wir können jetzt noch ein Item hinzufügen, dass überhaupt nicht der 2PL Gleichung folgt. Dafür überschreiben wir die answer-Werte von einem Item (z.B. Item 10) einfach mit zufälligen Werten:

```{r}

## Random theta Werte, unabhängig von Schwierigkeit
# sim_dat <- sim_dat %>%
#   mutate(p = ifelse(item_id == 10, exp(sin(a*(theta - b)))/(1 + exp(sin(a*(theta - b)))), p)) 
#   

## Item wird von hohen Theta falsch beantwortet und von niedrigem Theta richtig
