...
Unser finaler `data.frame` hat `r n_subj * 13` Zeilen, wie verlangt. Lasst uns auch noch einmal schauen, ob eine zufällige Person alle Items beantwortet hat:
...
Wir können jetzt auch ein bisschen die Antwortmuster manipulieren, um zu schauen, wie sich die Items verhalten, wenn bestimmte Auffälligkeiten vorliegen. Z.B. können wir festlegen, dass Item 3 von Leuten mit niederiger Fähigkeit eher richtig beantwortet wird, von allen anderen leuten aber eher falsch. 

```{r}

sim_dat <- sim_dat %>%
  mutate(p = ifelse(item_id == 3, ifelse(theta < -1.5, 0.95, 0.05), p))
