...
### Items
1. Zuerst die Items. Baue einen `data.frame` mit dem Namen `items`. Er soll `r `n_items` Zeilen und 4 Spalten haben und folgendes enthalten:

- `item_id`: Die ID des Items, von 1 bis `r n_items`` 
- `alpha`: Die Diskriminationsparameter, liegen zwischen 0.1 und 2.5 in `r `n_items` gleich langen Schritten.
