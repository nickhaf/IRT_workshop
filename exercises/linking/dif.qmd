...
# 2. Waldtest
Wir werden einen [Waldtest]() durchführen, einfach weil man hier dran die Grundidee nochmal ganz gut siet. Wie im [Theorieteil]() besprochen gibt es aber viele verschiedene Optionen, implementiert in diversen Paketen. Für diese Übung werden wir mit [equateIRT](https://cran.r-project.org/web/packages/equateIRT/vignettes/equateIRT_tutorial.html) arbeiten.  
...
tam_f <- tam(qa_dat_f[, 3:ncol(qa_dat_f)])
tam_m <- tam(qa_dat_m[, 3:ncol(qa_dat_m)])
...
dat_link <- TAM::tam.linking(list(tam_f, tam_m), type="Hae")
...
dif_dat <- data.frame(item = colnames(qa_dat_f[,3:ncol(qa_dat_f)]), 
...
library(ggplot2)
ggplot(data = dif_dat, aes(x = group_diff, y = item, colour = group_favour)) +
